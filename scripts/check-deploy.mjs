...
if (진입점있음) {
  ...
  // 5-b) wchain 쪽 게이트 (2026-08-22 신설)
  //    위 블록은 Llove/js 만 읽는다 — wchain/js 의 게이트는 지금껏 검사 밖이었다.
  //    "승인 없이 켜지 말 것"은 사람 눈에만 맡길 규칙이 아니므로, **봉인이어야 하는 게이트가
  //    실제로 false 인지**까지 확인한다(존재만 보는 위 검사보다 한 걸음 더 나간다).
  //    ※ 국어원_활성화는 이미 승인 하에 켜져 있는 게이트라 여기 넣지 않는다.
  {
    // 적절성검증_활성화는 2026-08-30 관리자님 승인으로 개방(true)돼 이 목록에서 뺐다 —
    // 봉인 검사는 "아직 승인 안 된 게이트"용이지, 승인된 게이트를 계속 false로 강제하는
    ...
    for (const [플래그, 경로] of 봉인이어야_하는_게이트) {
      ...
      확인(`${경로} 의 ${플래그} 가 봉인(false) 상태다`,
        new RegExp(`const\\s+${플래그}\\s*=\\s*false\\s*;`).test(본문));
    }
  }

  // 5-c) "그록" 문구 잔재 재발 방지 (2026-08-30) — 적절성 검증은 이제 Claude/GPT를 쓰는데
  //    로그 문구에 "그록"이 두 줄 남아 관리자님이 실기기에서 발견했다. 재발하지 않게 고정.
  {
    const 서바이벌 = existsSync('wchain/js/서바이벌.js') ? readFileSync('wchain/js/서바이벌.js', 'utf8') : '';
    확인('wchain/js/서바이벌.js 에 "그록" 문구가 남아 있지 않다', !서바이벌.includes('그록'));
  }
