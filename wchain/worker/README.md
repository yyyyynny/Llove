...
**🔧 2026-08-19 실측 후 수정** — 1차 배포분을 curl로 실측했더니 `뜻풀이그룹`이 항상 1개로
뭉쳐 나왔습니다("필연" 조회 시 必然·筆硯 구분 없이 뜻 3개가 한 그룹). 그룹화 키를 `sup_no`
1개만 보던 것을 `target_code` 우선 + `sup_no` 대체로 바꿨습니다(아래 "배포 후 확인" ③번으로
재검증 필요 — **이 수정을 다시 배포해야 반영됩니다**). 그래도 안 맞으면 `디버그:true`를 실어
보내면 원본 opendict item을 그대로 받아볼 수 있으니, 그 결과를 보고 한 번 더 고치면 됩니다.
