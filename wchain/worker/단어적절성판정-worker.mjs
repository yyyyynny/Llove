// '잇는' — 단어 적절성 판정 Worker (봉인 골격, 2026-08-22, 내부 구현은 xAI Grok API 호출)
//
// ⚠️ wchain(끝말잇기)은 지금까지 Grok을 전혀 쓰지 않는다 — 우리말샘 API 단일 체계로 동작한다.
// 이 파일은 그 위에 새로 얹는 기능이라, 이름도 구현 수단(Grok)이 아니라 기능(적절성 판정)을
// 따랐다 — 나중에 다른 모델로 바꿔도 이름이 거짓말이 되지 않도록.
//
// ── 왜 필요한가 ──────────────────────────────────────────────────────────
