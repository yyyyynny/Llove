...
// ⚠️ 미확정 — 관리자님이 모델 조합을 정하면 이 두 줄만 바꾸고 해당 시크릿을 등록하면 된다.
//    제공사는 위 제공사표의 키('openai호환' | 'anthropic' | 'google') 중 하나.
//    2차는 1차와 **다른 제공사**여야 교차검증 의미가 있다(같은 회사면 편향을 공유).
//    예) const 일차_설정 = { 제공사:'anthropic',  모델:'claude-haiku-4-5' };
//        const 이차_설정 = { 제공사:'openai호환', 모델:'gpt-5-nano', 추론:'low' };
const 일차_설정 = null;
const 이차_설정 = null;
  ...
  async fetch(request, env){
    ...
    try{
      const 결과 = await 단어_적절성_판정(env, {
        ...
        맥락: payload.맥락 || '',
        반박사유,
        // 서버에서도 한 번 더 자른다 — 클라이언트 제한만 믿지 않는다.
        반박보충: String(payload.반박보충 || '').slice(0, 100),
      });
