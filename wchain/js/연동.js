...
const 잇는_로컬키 = ['plx_잇는_국어원캐시_v2', 'plx_잇는_국어원후보캐시_v2', 'plx_잇는_테마연동'];
// 버전 접미사를 올리기 전에 저장된 옛 캐시들 — 목록을 일일이 늘리는 대신 접두사로 쓸어 담는다.
const 잇는_로컬접두 = 'plx_잇는_';

function 잇는_로컬삭제(){
  try{
    for(const k of 잇는_로컬키) localStorage.removeItem(k);
    // 접두사가 같은 키는 전부 잇는 것이다 — 과거 버전 키가 남아 "삭제했는데 그대로"가 되지 않게.
    const 지울것 = [];
    for(let i = 0; i < localStorage.length; i++){
      const k = localStorage.key(i);
      if(k && k.startsWith(잇는_로컬접두)) 지울것.push(k);
    }
    for(const k of 지울것) localStorage.removeItem(k);
...
function 게임데이터삭제_실행(){
  ...
  if(fbAuth && fbAuth.currentUser && fbDb){
    ...
    //
    // ⚠️ 2026-07-29: 종전에는 이 안내를 로그_추가로 띄웠는데, 삭제 버튼은 **페르소나 선택 화면**에
    // 있어서 플레이 화면의 로그창이 보이지 않는다 — 아무도 못 읽는 안내였다. 모달로 바꾼다.
    // 확인을 누르면 이동하므로 안내를 놓칠 일도 없다.
    게임데이터_확인모달('ℹ️ 이 기기의 기록만 삭제되었습니다',
      '로그인되어 있지 않아 서버(계정) 기록에는 손대지 않았습니다.<br><br>'
      + '이 기기에 저장된 잇는 캐시·설정만 지웠습니다. 계정으로 로그인한 뒤 다시 삭제하시면 '
      + '포탈 해금 기록까지 되돌릴 수 있습니다.',
      '학습 세계로', 학습세계로);
  }
}
