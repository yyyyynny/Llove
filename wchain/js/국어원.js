...
// '뜻 보기' 버튼(서바이벌.js)용 — 국어원_단어조회_상세()의 뜻풀이그룹을 로그 창에 한 줄씩
...
function 뜻풀이_로그줄들(결과){
  ...
  const 그룹 = 결과.뜻풀이그룹;
  if(그룹.length === 1) return [그룹[0].뜻풀이.join(' / ')];
  return 그룹.map((g, i) => `${뜻풀이_동그라미[i] || `(${i + 1})`} ${g.뜻풀이.join(' / ')}`);
}

/* 후보 캐시 키에도 버전을 붙인다(2026-07-29 _v2).
   ────────────────────────────────────────────────────────────────
   종전 키에는 버전이 없어서, Worker가 돌려준 후보 목록이 **영구히** 기기에 남았다.
   지금 Worker는 붙임표가 든 표제어를 버리고 첫 페이지만 가져와 '사'에 후보가 2개뿐인데
   (Worker_수정요청.md ②), 그 빈약한 목록이 캐시에 박히면 **Worker를 고쳐도 그 기기에서는
   영원히 2개짜리 게임이 된다.** 버전을 올려 옛 캐시를 자연히 무시하게 하고, 앞으로 Worker가
...
async function 국어원_후보목록조회(글자, 방향){
  ...
  // 빈 목록은 캐시하지 않는다(2026-07-29). "이 글자로 이을 단어가 정말 하나도 없다"는 결론은
