...
`style.css` + `js/` 25개 분할 스크립트로, 빌드 없이 GitHub Pages에 그대로 배포된다.
...
Llove 홈에서 진입하는 별도 끝말잇기 게임(`wchain/`, 같은 도메인이라 로그인 자동 공유).
서바이벌·아케이드 두 모드, 국립국어원 우리말샘 API 연동(Cloudflare Worker 프록시, 실배포
검증 완료)까지 갖췄다. 설계·게이트 정책은 [`wchain/시스템.md`](wchain/시스템.md) 참조.
...
- `작업인계_노트.md` — 세션 인계 노트
- `wchain/시스템.md` — '잇는'(끝말잇기 이세계) 설계·게이트 정책
