...
/* ⚠️⚠️ 임시 하드코딩 — data/ DB 채움 완료로 더 이상 필수 아님 ⚠️⚠️
   [사유] 최초 작성 당시엔 data/고사성어속담.json·data/한자우리말.json 이 빈 파일이라
          내장 풀(FC_GOSAEONGEO·FC_HANJA)이 각 1건뿐이라 오답 보기 3개 구성이 불가능했다.
          (가상 데이터 아님 — 실제 뜻이지만 임시 배치)
   [현재 상태] 재구조화 세션 콘텐츠 라운드 1~8로 두 파일 모두 각 80개 채움 완료.
          sq2_출제풀()이 DB 풀을 우선 사용하므로 이 두 임시 배열은 이제 안전하게 제거 가능
          (제거 안 해도 무해 — 단어 중복은 자동 배제됨).
