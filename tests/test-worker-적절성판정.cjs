...
const { 제공사표, 반박사유표, 시스템프롬프트, 사용자프롬프트, AI_판정, 반박보충_바이트초과인가 } = new Function(`
  ${함수부}
  return { 제공사표, 반박사유표, 시스템프롬프트, 사용자프롬프트, AI_판정, 반박보충_바이트초과인가 };
`)();
...
async function main() {
  ...
  // (6) 반박보충 500바이트 초과 — 2026-08-30: curl로 직접 750바이트를 보내봤더니(재배포된
  // 구버전 Worker) 조용히 잘려서 정상 처리(HTTP 200)됐다. 클라이언트는 이미 500바이트에서
  // 막는데(반박_기타제출), 서버가 조용히 잘라서 통과시키면 그 상한을 curl 등으로 우회할 수
  // 있어 무의미해진다 — 그래서 잘라서 통과가 아니라 **명확히 거부(400)**하도록 고쳤다.
  // 진입점(fetch)은 이 함수를 호출해 400을 돌려주는데, 진입점 자체는 다른 Worker
  // 테스트들과 같은 이유로(Cloudflare 전용) 여기서 직접 실행하지 않으므로 판정 로직만
  // 고정한다 — ⚠️ 이 버전의 실제 400 응답은 아직 재배포 전이라 curl로 재확인 못 했다.
  // 재배포 후 반드시 실측할 것.
  {
    assert('500바이트 이하는 초과가 아니다', !반박보충_바이트초과인가('가'.repeat(166)));   // 498바이트
    assert('500바이트 정확히는 초과가 아니다', !반박보충_바이트초과인가('a'.repeat(500)));   // 영문 1바이트×500
    assert('500바이트를 1바이트라도 넘으면 초과다', 반박보충_바이트초과인가('a'.repeat(501)));
    assert('한글 기준 500바이트 초과(가 167개=501바이트)도 잡는다',
      반박보충_바이트초과인가('가'.repeat(167)));
    assert('빈 문자열은 초과가 아니다', !반박보충_바이트초과인가(''));
    assert('진입점이 초과 시 400과 안내 문구를 돌려준다',
      src.includes("반박보충이 500바이트를 넘습니다") && src.includes('400, origin'));
    assert('진입점이 더 이상 잘라서 통과시키지 않는다(slice·자르기 호출 없음)',
      !/단어_적절성_판정\(env,\s*\{[\s\S]*?반박보충:\s*[^,}]*(slice|자르기)/.test(src));
  }
