...
async function main() {
  ...
  // (5) 제공사 미확정 상태 — 관리자님이 아직 모델을 안 정했으므로 null이어야 한다.
  //     실수로 아무 값이나 채워 배포되는 걸 막는 가드(진입점이 500을 돌려준다).
  {
    assert('일차_설정이 아직 미확정(null)이다', /const 일차_설정 = null;/.test(src));
    assert('이차_설정이 아직 미확정(null)이다', /const 이차_설정 = null;/.test(src));
    assert('진입점이 미설정을 500으로 명확히 막는다',
