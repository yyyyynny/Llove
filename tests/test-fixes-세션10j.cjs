// 세션10-j — 창조주 업적 시도(오타·부분 복사 등 미달성 상태의 【】 포함 메시지) 채팅 기록 저장 방지
const { load, makeHarness } = require('./load.cjs');
load((window) => {
  const { assert, finish } = makeHarness('세션10-j 창조주 시도 채팅 미저장 테스트');
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        /* ── 창조주 이미 달성 후에는 【】 포함해도 정상 저장(과거 인용 등 정상 대화 보존) ── */
        ev('사용자.창조주달성 = true;');
        ask('아까 봤던 【이 문장】이 무슨 뜻이었지?');
        setTimeout(() => {
          const s4 = ev('현재채팅세션');
          assert('창조주 달성 후에는 【】 포함 메시지도 정상 저장됨', !!s4 && s4.메시지.some(m => m.내용.includes('【이 문장】')));
