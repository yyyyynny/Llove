...
async function main(){
  ...
  console.log('\n[12] 난이도 차등');
  {
    ...
    // 2026-07-29 실수 폐지 — 틀리면 곧바로 목숨 -1이라 목숨을 종전의 4배 선으로 올렸다.
    확인('난이도별 목숨/힌트가 다르다 (안온10/5 · 격동7/3 · 초월5/2 · 심연3/1)',
         ['안온','격동','초월','심연'].map(자원).join(' ') === '10/5 7/3 5/2 3/1',
         ['안온','격동','초월','심연'].map(자원).join(' '));
    const 아케 = ctx.새게임상태(); 아케.game_mode = 'ARCADE'; 아케.diff = '심연';
    ctx.reset_game(아케);
    확인('아케이드는 난이도와 무관하게 목숨8·힌트3 고정',
         아케.hearts === 8 && 아케.hints === 3);
