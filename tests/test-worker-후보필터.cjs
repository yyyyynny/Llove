...
// ── 동사·형용사 제외 (2026-08-30, 관리자님 실기기 제보 — AI가 "사나워지다" 냄) ─────────
// 배포된 우리말샘 Worker에 디버그 모드로 직접 조회해 확인한 실측 pos 값(README.md 기록).
const 품사_실측 = {
  '사나워지다': [{ type: '일반어', pos: '동사' }],
  '먹다': [{ type: '일반어', pos: '동사' }],
  '달리다': [{ type: '일반어', pos: '동사' }],
  '예쁘다': [{ type: '일반어', pos: '형용사' }],
  '아름답다': [{ type: '일반어', pos: '형용사' }],
  '사과': [{ type: '일반어', pos: '명사' }],   // 대조군 — 포함돼야 함
};
for(const [단어, sense] of Object.entries(품사_실측)){
  const 결과 = 후보_부적절한가({ word: 단어, sense });
  const 기대_제외 = 단어 !== '사과';
  assert(`${단어}(${sense[0].pos}): ${기대_제외 ? '제외돼야 함' : '포함돼야 함'}`,
    결과 === 기대_제외, `실제=${결과 ? '제외' : '포함'}`);
}
assert('동사·형용사가 뒤쪽 sense에만 있으면 첫 sense(명사) 기준으로 포함',
  !후보_부적절한가({ word: '가장', sense: [{ type: '일반어', pos: '명사' }, { type: '일반어', pos: '동사' }] }));
